...
import { Sparkles, Gem, LayoutGrid, Square } from 'lucide-react';
import { motion, LayoutGroup } from 'framer-motion';

interface DiamondJewelleryPageProps {
...
const DiamondJewelleryPage: React.FC<DiamondJewelleryPageProps> = ({ 
  onProductClick, 
  onQuickView, 
  t,
  isProductWishlisted,
  ...
  const subCategories = ['All', 'Necklaces', 'Rings', 'Earrings', 'Bracelets', 'Others'];

  const diamondProducts = useMemo(() => {
    let products = MOCK_PRODUCTS.filter(p => 
      p.isDiamond && (activeSubCategory === 'All' || p.category === activeSubCategory)
    );
    ...
    <div className="min-h-screen">
      ...
      <section className="mb-16">
        <div className="relative h-[40vh] lg:h-[60vh] rounded-3xl lg:rounded-5xl overflow-hidden shadow-2xl group">
          <img 
            ...
            alt="Diamond Radiance" 
          />
          <div className="absolute inset-0 bg-charcoal/20 flex items-center px-6 lg:px-24">
            <div className="max-w-xl">
              <div className="flex items-center gap-3 mb-4 lg:mb-6 animate-fade-in">
                <div className="w-10 h-[1px] bg-gold" />
                <span className="text-[9px] font-sans font-bold uppercase tracking-[0.4em] text-white">The Brilliance Series</span>
              </div>
              <h1 className="text-4xl lg:text-8xl font-serif text-white mb-4 lg:mb-8 leading-tight tracking-tighter">{t.diamond_brilliance}</h1>
              <p className="text-white/90 text-sm lg:text-xl font-serif italic max-w-lg leading-relaxed hidden sm:block">
                Precision cut for maximum brilliance. Discover curated solitaires and high-clarity diamonds.
              </p>
            </div>
          </div>
        </div>
      ...
      <section className="mb-12">
        <div className="flex flex-col items-center">
          <h2 className="text-[10px] font-sans font-bold uppercase tracking-[0.3em] text-gold mb-6">{t.refine_archive}</h2>
            ...
            {subCategories.map((cat) => (
              <button
                ...
                className={`px-6 py-2.5 rounded-full text-[9px] font-sans font-bold uppercase tracking-widest transition-all duration-500 border ${
                  ...
                  : 'bg-white text-charcoal/60 border-cream hover:border-gold/30 hover:text-gold'
                }`}
              >
      ...
      <section className="pb-20">
        <div className="flex flex-col sm:flex-row justify-between items-center mb-10 border-b border-cream pb-6 gap-4">
          <div className="flex items-center gap-3 w-full sm:w-auto">
             ...
             <span className="px-3 py-1 bg-gold/5 rounded-full text-[8px] font-sans font-bold uppercase text-gold tracking-widest">{diamondProducts.length} {t.results}</span>
        ...
        {diamondProducts.length > 0 ? (
          ...
          <div className="py-24 flex flex-col items-center justify-center text-center">
            <Gem size={40} className="text-gold/20 mb-4" />
