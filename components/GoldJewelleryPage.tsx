...
const GoldJewelleryPage: React.FC<GoldJewelleryPageProps> = ({ 
  onProductClick, 
  onQuickView, 
  t,
  isProductWishlisted,
  ...
  const subCategories = ['All', 'Necklaces', 'Rings', 'Earrings', 'Bangles', 'Chains'];

  const goldProducts = useMemo(() => {
    let products = MOCK_PRODUCTS.filter(p => 
      p.metal === 'Gold' && (activeSubCategory === 'All' || p.category === activeSubCategory)
    );
      ...
      <section className="mb-16">
        <div className="relative h-[40vh] lg:h-[60vh] rounded-3xl lg:rounded-5xl overflow-hidden shadow-2xl group">
          <img 
            ...
            alt="Gold Heritage" 
          />
          <div className="absolute inset-0 bg-gradient-to-r from-charcoal/60 via-charcoal/20 to-transparent flex items-center px-6 lg:px-24">
            <div className="max-w-xl">
              <div className="flex items-center gap-3 mb-4 lg:mb-6">
                <div className="w-10 h-[1px] bg-gold" />
                <span className="text-[9px] font-sans font-bold uppercase tracking-[0.4em] text-gold">Heritage Series</span>
              </div>
              <h1 className="text-4xl lg:text-8xl font-serif text-white mb-4 lg:mb-8 leading-tight tracking-tighter">{t.gold_heritage}</h1>
              <p className="text-white/80 text-sm lg:text-xl font-serif italic max-w-lg leading-relaxed hidden sm:block">
                Celebrate timeless craftsmanship with signature high-purity gold collections.
              </p>
            </div>
          </div>
        </div>
      ...
      <section className="pb-20">
        <div className="flex flex-col sm:flex-row justify-between items-center mb-10 border-b border-cream pb-6 gap-4">
          <div className="flex items-center gap-3 w-full sm:w-auto">
             ...
             <span className="px-3 py-1 bg-gold/5 rounded-full text-[8px] font-sans font-bold uppercase text-gold tracking-widest">{goldProducts.length} {t.results}</span>
        ...
        {goldProducts.length > 0 ? (
          ...
          <div className="py-24 flex flex-col items-center justify-center text-center">
            <Sparkles size={40} className="text-gold/20 mb-4" />
